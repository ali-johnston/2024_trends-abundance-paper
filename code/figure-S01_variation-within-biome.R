...
data_dir <- "data"
outputs_dir <- "outputs"
figures_dir <- path("figures", "figure-S01")
...
tr_sp_bcr <- trends |>
  ...
  summarise(tr_min = min(abd_ppy_median),
            tr_max = max(abd_ppy_median),
            no_cells = n(),
            .groups = "drop") |>
...
plot_loc <- path(figures_dir, "figure-S01_histogram_species-bcr-ranges.tif")
tiff(plot_loc, width = 12, height = 8, units = "cm", pointsize = 9, res = 600)
par(mar = c(5, 5, 1, 1))

hist(tr_sp_bcr$tr_range, breaks = h1$breaks, col = "grey70", 
     xlim = c(0, 40),
     xlab = "Range of single-species trends within each BCR (percent-per-year)",
     xaxt = "n", yaxt = "n", main = "")
axis(side = 1, at = c(0, 10, 20, 30, 40))
axis(side = 2, at = c(0, 1000, 2000))
hist(tr_sp_bcr$tr_range[tr_sp_bcr$at_least_20==1],
     breaks = h1$breaks, col = "grey30", add = TRUE)
segments(x0 = med_all, x1 = med_all, y0 = 0, y1 = 3000, col = "white", lty = 2)

legend(x = 20, y = 2000, fill = c("grey70", "grey30"), 
       legend = c("BCR-species < 20 cells", "BCR-species >= 20 cells"),
       cex = 0.8)
